...
class RMQ {
 ...
 public:
  static RMQ uninitialized() { return RMQ(); }

  template <class Container>
  void init(Container &&src, Cmp &&cmp) {
    data = std::forward<Container>(src);
    ...
    for (size_t i = 0; i < blocks.size(); ++i)
      sparse_table[0][i] =
          Idx(blocks[i].init(*this, i * W, std::min(i * W + W, data.size())) +
              W * i);
    ...
    for (int lvl = 0; lvl + 1 < (int)sparse_table.size(); ++lvl, w *= 2) {
      ...
      for (size_t i = 0; i < sparse_table[lvl + 1].size(); ++i)
        sparse_table[lvl + 1][i] =
            Idx(pick_index(sparse_table[lvl][i], sparse_table[lvl][i + w]));
    }
  }
  ...
  size_t argmin(size_t i, size_t j) const {
    ...
    if (block_i == block_j) return blocks[block_i](i % W, j % W) + W * block_i;

    size_t sides_ans = pick_index(blocks[block_i](i % W, W - 1) + W * block_i,
                                  blocks[block_j](0, j % W) + W * block_j);

    if (block_i + 1 == block_j) return sides_ans;
  ...
  struct block {
    ...
    int init(RMQ &rmq, size_t begin, size_t end) {
      ...
      for (int i = 1; i < n; ++i) {
        ...
        while (snapshots[i] &&
               rmq.compare_indices(begin + i, begin + stack.back())) {
      ...
      for (int i = n; i < W; ++i) snapshots[i] = snapshots[i - 1];
